# Method using Shoelace and Pick's theorem instead of graphical methods.
...
# Use Pick's theorem to get the number of points inside
# Area uses shoelace formula, i.e. determinant of every pair of points on edge
area = 0.5 * sum([selected_route[k][0]*selected_route[k+1][1] - selected_route[k][1]*selected_route[k+1][0] for k in range(len(selected_route)-1)]) 
boundary_length = len(set(selected_route))
number_inside = area - (boundary_length / 2) + 1
