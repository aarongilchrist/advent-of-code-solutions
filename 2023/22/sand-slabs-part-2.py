...
# Find bricks below
below_item_dict = {}
for k in brick_values.keys():
    below_items = []
    for y, above in above_item_dict.items():
        if k in above:
            below_items.append(y)
    below_item_set = set(below_items)
    below_item_dict.update({k:below_item_set})

# Look for bricks above which will be affected by bricks falling
affected_bricks = set(brick_values.keys()).difference(disintegrated_bricks)
number_falling_dict = {}
for brick in affected_bricks:
    above = set([brick])
    prev_above = set()
    while len(above) != len(prev_above):
        prev_above = above
        bricks_to_add = []
        for item in above:
            for item_brick in above_item_dict[item]:
                if np.prod([1 if brick in above else 0 for brick in below_item_dict[item_brick]]) == 1:
                    bricks_to_add.append(item_brick)
        above = above.union(set(bricks_to_add))
    falling_bricks = set(above).difference([brick])
    number_falling_dict.update({brick:falling_bricks})
#print(number_falling_dict)

total_number_falling = sum([len(x) for x in number_falling_dict.values()])

print(total_number_falling)